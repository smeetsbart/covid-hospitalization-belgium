...
for i,R0i in enumerate(R0):
   for j,IFRi in enumerate(IFR):
      ...
      Ih[i,j] = 1.-1./R0i if results['rsq'] > th_rsq else np.nan
...
Ih = np.ma.masked_where( np.isnan(Ih), Ih)
Ifh = If / Ih
...
fig = plt.figure(figsize=(6,5))
ax=fig.add_subplot(111)
cmap = cm.get_cmap('RdYlGn', 11)    # 11 discrete colors
im = ax.pcolormesh(R0, IFR*100, T(Ifh)*100, cmap=cmap,vmin=15,vmax=70)
#CS = ax.contour(R0,IFR*100, T(If)*100, levels=[10,15,20,30,40], colors='k')
#ax.clabel(CS, inline=1, fontsize=10,fmt='%1.0f')
CS = ax.contour(R0,IFR*100, T(Q), levels=[0.3,0.4,0.5,0.6,0.7,0.75], colors='k',linestyles='--', alpha=0.75)
ax.clabel(CS, inline=1, fontsize=10,fmt='Q=%1.2f')
plt.plot( [R0.min(),R0.max()],[1.25,1.25], ls='--', lw=2, color='w')
#plt.plot( [R0.min(),R0.max()],[0.75,0.75], ls='--', lw=2, color='w')
plt.plot( [R0_France], [IFR_France], marker='x', mew=3, ms=13, color='w',ls='None')
plt.annotate( "Roques et al. (2020)\n(France)", ( 3.4,0.75 ), color='w', fontsize=13,fontstyle='italic' )
ax.contourf( R0, IFR*100, T(RSQ_c), levels=[RSQ_th,1], colors='w',linestyles='-',alpha=0.25)
ax.contour( R0, IFR*100, T(RSQ_c), levels=[RSQ_th], colors='w',linestyles='-',alpha=0.75)
plt.annotate( "Molenbergs et al. (2020)", (R0.mean()-0.33*(R0.max()-R0.mean()), 1.28), color='w', fontsize=13,fontstyle='italic' )

#plt.annotate('Best data fit', xy=(4.0, 0.45),  xycoords='data',
            #xytext=(3.7, 0.3), textcoords='data', fontsize=13,
            #arrowprops=dict(facecolor='black', shrink=0.1,edgecolor='None', width=2, headwidth=8,headlength=8),
            #horizontalalignment='right', verticalalignment='top',
            #)

#plt.annotate( "Campbell et al. (2020) (Netherlands)", (R0.mean()-0.33*(R0.max()-R0.mean()), 0.6), color='', fontsize=13,fontstyle='italic' )
cbar = fig.colorbar(im, ax=ax)
cbar.set_label("Fraction herd immunity (%)",fontsize=16)
ax.set_xlabel( "$R_0$", fontsize=16 )
ax.set_ylabel( "IFR (%)", fontsize=16)
plt.tight_layout()
